# LiveData.py — streamlined watchlist (fast batched fetch, 2-dec ceil rounding, safe Sheets)
...
# ============== CONFIG ==============
st.set_page_config(page_title="Multi-Portfolio Dashboard", page_icon="📊", layout="wide", initial_sidebar_state="expanded")

# subtle tint for ticker editor
st.markdown("""
<style>
[data-testid="stDataEditor"] table tr td:first-child,
[data-testid="stDataEditor"] table tr th:first-child { background-color: rgba(138,43,226,0.08); }
</style>
""", unsafe_allow_html=True)

SHEET_TAB_NAME = "Watchlist"

# ============== UTIL ==============
def colored_header_bg(title: str, bg_color: str = "#8A2BE2", text_color: str = "white",
                      font_size: int = 26, align: str = "center", gradient: bool = True):
    gradient_css = f"linear-gradient(135deg, {bg_color} 0%, {bg_color}AA 100%)" if gradient else bg_color
    st.markdown(f"""
        <div style="
            background: {gradient_css}; color:{text_color}; padding:14px; border-radius:12px;
            font-size:{font_size}px; font-weight:700; text-align:{align};
            box-shadow:0 2px 8px rgba(0,0,0,0.2); margin:10px 0 20px; letter-spacing:.5px;">
            {title}
        </div>
    """, unsafe_allow_html=True)

def round2_up(x):
    """Ceil-style rounding to 2 decimals."""
    try:
...
def signature(df: pd.DataFrame) -> int:
    return hash(tuple(df["Ticker"].tolist())) if "Ticker" in df.columns else 0
...
def _assert_sheets_secrets():
    if "sheets" not in st.secrets:
        st.error("Missing [sheets] in secrets."); st.stop()
    s = st.secrets["sheets"]
    for k in ("sheet_id", "service_account"):
        if k not in s:
            st.error(f"Missing key in [sheets]: {k}"); st.stop()
...
def get_sheet_client():
    ...
    if isinstance(raw, dict):
        ...
        def _escape_pk_newlines(s: str) -> str:
            return re.sub(r'("private_key"\s*:\s*")([^"]+?)(")',
...
def save_watchlist_to_sheet(df: pd.DataFrame, prevent_empty: bool = True) -> bool:
    ...
    if prevent_empty and clean.empty:
        st.warning("Skipped save: watchlist is empty."); return False
    client = get_sheet_client()
    ws = _open_or_create_worksheet(client, st.secrets["sheets"]["sheet_id"], SHEET_TAB_NAME)
    data = [["Ticker"]] + [[t] for t in clean["Ticker"].tolist()]
    try: ws.batch_clear(["A2:A"])
...
def fetch_names_fast(tickers: List[str]) -> Dict[str, str]:
    ...
    for t in _tickers_norm(tickers):
        ...
        try:
            ...
            n = info.get("shortName") or info.get("longName") or info.get("name") or ""
        except Exception:
            pass
        names[t] = n
    return names
...
if "watchlist" not in st.session_state:
    try:
        df0 = get_sheet_snapshot() if sheets_configured() else pd.DataFrame({"Ticker": ["QQQ","AAPL","MSFT"]})
        if df0 is None or df0.empty: df0 = pd.DataFrame({"Ticker": ["QQQ","AAPL","MSFT"]})
...
if "watchlist_saved_sig" not in st.session_state:
    try: st.session_state["watchlist_saved_sig"] = signature(get_sheet_snapshot())
    except Exception: st.session_state["watchlist_saved_sig"] = signature(st.session_state["watchlist"])
...
# --- Small editor JUST for tickers (avoids duplicate big table) ---
tick_edit = st.data_editor(
    st.session_state["watchlist"],
    key="watchlist_editor_tickers",
    num_rows="dynamic",
    column_config={"Ticker": st.column_config.TextColumn(help="e.g., MSFT, ETH-USD, XAUUSD=X")},
    width=400,
)
st.session_state["watchlist"] = normalize_watch_df(tick_edit)
st.session_state["_wl_last_edit_ts"] = time.time()

# --- Build performance view (single, colored table) ---
...
last_price, prev_close, history = bundle.get("last_price", {}), bundle.get("prev_close", {}), bundle.get("history", {})
...
# render ONE colored performance table
styled = (
...
with c1:
    if st.button("💾 Save Now"):
        ...
        if wl.empty:
            st.warning("Won’t overwrite with an empty list.")
        elif save_watchlist_to_sheet(wl, prevent_empty=True):
            st.success("Saved to Google Sheets.")
            st.session_state["watchlist_saved_sig"] = signature(wl)
...
last_saved = st.session_state.get("watchlist_saved_sig")
if (sheets_configured() and last_saved is not None and current_sig != last_saved
